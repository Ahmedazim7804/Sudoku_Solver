...
class Ui(qtw.QMainWindow):
    def __init__(self):
        ...
        if not flag:
            qtw.QMessageBox.critical(self,"Sudoku Solver","Model.py File Not Found")
            sys.exit()
        #Load Uic
        try:
            uic.loadUi("sudoku.ui", self)
        except FileNotFoundError:
            qtw.QMessageBox.critical(self,"Sudoku Solver",f"Sudoku.ui File Not Found",qtw.QMessageBox.Ok)
            sys.exit()
        ...
        dock_widget.layout().addWidget(self.text_edit,0,0)
        dock_widget.layout().addWidget(self.submit_button,1,0)
